...
object Interaction2 {
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
      Signal {
        ...
        val yearSelection: Int = sliderValue()
        if (yearSelection < range.start) {
          range.start
        } else if (yearSelection > range.end) {
          range.end
        } else {
          yearSelection
        }
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = {
    Signal(s"target/${selectedLayer().layerName.id.toLowerCase}/${selectedYear()}/{x}/{y}.png")
  }
